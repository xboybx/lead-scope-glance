...
const horizonConfig: Record<
  TimeHorizon,
  { label: string; sublabel: string; Icon: typeof Clock; description: string }
> = {
  immediate: {
    label: 'Immediate',
    sublabel: 'Due today or overdue',
    Icon: Clock,
    description: 'Work items that require immediate attention.',
  },
  'short-term': {
    label: 'Short-term',
    sublabel: 'Due this week',
    Icon: Calendar,
    description: 'Work items that are due in the near future.',
  },
  upcoming: {
    label: 'Upcoming',
    sublabel: 'Due next week+',
    Icon: CalendarDays,
    description: 'Work items that are planned for the upcoming weeks.',
  },
};

export function TimeHorizonCard({ horizon }: TimeHorizonCardProps) {
  const summary = getHorizonSummary(horizon);
  const { label, sublabel, Icon, description } = horizonConfig[horizon];
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            className={cn(
              'cursor-pointer rounded-xl bg-card p-5 shadow-md transition-all duration-200 hover:shadow-lg animate-fade-in',
              hasIssues &&
                horizon === 'immediate' &&
                'ring-2 ring-status-at-risk/30'
            )}
          >
